...
magick convert \
    ...
    -gravity SouthEast -pointsize 20 -annotate +20+20 "${the_date}" \
    -gravity SouthWest -pointsize 20 -annotate +20+20 "$(cat ${FREESURFER_HOME}/build-stamp.txt)" \
    -gravity NorthWest -pointsize 20 -annotate +20+50 "${label_info}" \
    page1.png
